...
export function RouteCard({ route }) {
  return (
    <article
      className="relative overflow-hidden rounded-[24px] border border-slate-200 bg-slate-50/70 p-4"
      style={{
    ...
    >
      ...
      <div className="ml-2 mt-4 flex items-start gap-3 rounded-[20px] border border-white/80 bg-white/80 p-3">
        <Navigation className="mt-0.5 h-4 w-4 text-slate-500" />
        <p className="text-sm leading-6 text-slate-600">
          途經策略：<span className="font-medium text-slate-900">{route.label}</span>
        </p>
      </div>
    </article>
  );
